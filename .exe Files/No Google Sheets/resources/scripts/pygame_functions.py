...
# Third party library - some functions extended while others modified - please see helper_functions.py
...
class newTextBox(pygame.sprite.Sprite):       
    def __init__(self, text, xpos, ypos, width, case, maxLength, fontSize, font, fontColor, background):
        pygame.sprite.Sprite.__init__(self)
        self.text = ""
        self.width = width
        ...
        self.rect = self.image.get_rect()
        if font == 'visitor1':    
            self.fontFace = os.path.join(fonts_path, 'visitor1.ttf')
        elif font == 'pxlxxl':
            self.fontFace = os.path.join(fonts_path, 'pxlxxl.ttf')
        elif font == '04B_03__':
            self.fontFace = os.path.join(fonts_path, '04B_03__.ttf')
        else:
            self.fontFace = pygame.font.match_font("Arial")
        self.fontColour = fontColor
...
# use to stop music, play sound and then restart music
def playSoundAndWait(sound, wait=1000, loop=20):
    sound.play()
...
def makeTextBox(xpos, ypos, width, case=0, startingText="Please type here", maxLength=0, fontSize=22, font="pxlxxl", fontColour="black", background="white"):
    thisTextBox = newTextBox(startingText, xpos, ypos, width, case, maxLength, fontSize, font, fontColour, background)
